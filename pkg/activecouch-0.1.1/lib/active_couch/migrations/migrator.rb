  ...
  class Migrator
    class << self # Class methods
      ...
      def down(site, migration)
        ...
        # Migration for a view with name 'by_name' and database 'activecouch_test' should be PUT to
        # http://#{host}:#{port}/activecouch_test/_design/by_name.
        response = conn.delete("/#{migration.database}/_design/#{migration.view}", migration.view_js)
        case response.code
        when '201'
          true # 201 = success
        else
          raise ActiveCouch::MigrationError, "Error Downgrade view - got HTTP response #{response.code}"
        end
      end
